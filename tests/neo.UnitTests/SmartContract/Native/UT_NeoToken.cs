...
namespace Neo.UnitTests.SmartContract.Native
{
    ...
    public class UT_NeoToken
    {
        ...
        [TestMethod]
        public void Check_Name() => NativeContract.DVITA.Name.Should().Be(nameof(DvitaToken));

        [TestMethod]
        public void Check_Symbol() => NativeContract.DVITA.Symbol(_snapshot).Should().Be("DVITA");

        [TestMethod]
        public void Check_Decimals() => NativeContract.DVITA.Decimals(_snapshot).Should().Be(0);
        ...
        public void Check_RegisterValidator()
        {
            ...
            var members = NativeContract.DVITA.GetCandidates(snapshot);
        ...
        public void Check_UnregisterCandidate()
        {
            ...
            members = NativeContract.DVITA.GetCandidates(snapshot);
        ...
        public void Check_GetCommittee()
        {
            ...
            snapshot.Add(CreateStorageKey(20, G_Account), new StorageItem(new NeoAccountState()));
            var accountState = snapshot.TryGet(CreateStorageKey(20, G_Account)).GetInteroperable<NeoAccountState>();
            accountState.Balance = 200000000;
            ...
            var committeemembers = NativeContract.DVITA.GetCommittee(snapshot);
            ...
            committeemembers = NativeContract.DVITA.GetCommittee(snapshot);
        ...
        public void Check_Transfer()
        {
            ...
            NativeContract.DVITA.Transfer(snapshot, from, to, BigInteger.One, false, persistingBlock).Should().BeFalse(); // Not signed
            NativeContract.DVITA.Transfer(snapshot, from, to, BigInteger.One, true, persistingBlock).Should().BeTrue();
            NativeContract.DVITA.BalanceOf(snapshot, from).Should().Be(999999999);
            NativeContract.DVITA.BalanceOf(snapshot, to).Should().Be(1);

            var (from_balance, _, _) = GetAccountState(snapshot, new UInt160(from));
            var (to_balance, _, _) = GetAccountState(snapshot, new UInt160(to));

            from_balance.Should().Be(999999999);
            ...
            NativeContract.DVITA.Transfer(snapshot, to, from, BigInteger.One, true, persistingBlock).Should().BeTrue();
            ...
            Assert.ThrowsException<ArgumentOutOfRangeException>(() => NativeContract.DVITA.Transfer(snapshot, from, to, BigInteger.MinusOne, true, persistingBlock));
            Assert.ThrowsException<ArgumentOutOfRangeException>(() => NativeContract.DVITA.Transfer(snapshot, new byte[19], to, BigInteger.One, false, persistingBlock));
            Assert.ThrowsException<ArgumentOutOfRangeException>(() => NativeContract.DVITA.Transfer(snapshot, from, new byte[19], BigInteger.One, false, persistingBlock));

            // More than balance

            NativeContract.DVITA.Transfer(snapshot, to, from, new BigInteger(2), true, persistingBlock).Should().BeFalse();
        }
        ...
        public void Check_BalanceOf()
        {
            ...
            NativeContract.DVITA.BalanceOf(snapshot, account).Should().Be(1_000_000_000);

            account[5]++; // Without existing balance

            NativeContract.DVITA.BalanceOf(snapshot, account).Should().Be(0);
        }
        ...
        public void TestCalculateBonus()
        {
            ...
            Action action = () => NativeContract.DVITA.UnclaimedGas(snapshot, UInt160.Zero, 10).Should().Be(new BigInteger(0));
            ...
            action = () => NativeContract.DVITA.UnclaimedGas(snapshot, UInt160.Zero, 10).Should().Be(new BigInteger(0));
            snapshot.Delete(key);
            ...
            NativeContract.DVITA.UnclaimedGas(snapshot, UInt160.Zero, 100).Should().Be(new BigInteger(0));
            snapshot.Delete(key);
            ...
            NativeContract.DVITA.UnclaimedGas(snapshot, UInt160.Zero, 100).Should().Be(new BigInteger(0.5 * 100 * 10));
            snapshot.Delete(key);
            ...
            snapshot.Add(new KeyBuilder(NativeContract.DVITA.Id, 23).Add(ProtocolSettings.Default.StandbyCommittee[0]).AddBigEndian(uint.MaxValue - 50), new StorageItem() { Value = new BigInteger(50 * 10000L).ToByteArray() });
            NativeContract.DVITA.UnclaimedGas(snapshot, UInt160.Zero, 100).Should().Be(new BigInteger(50 * 10));
            snapshot.Delete(key);
        }
        ...
        public void TestGetNextBlockValidators1()
        {
            ...
            var result = (VM.Types.Array)NativeContract.DVITA.Call(snapshot, "getNextBlockValidators");
        ...
        {
            var snapshot = _snapshot.CreateSnapshot();
            var result = NativeContract.DVITA.GetNextBlockValidators(snapshot, 7);
        ...
        {
            var snapshot = TestBlockchain.GetTestSnapshot();
            var array = (VM.Types.Array)NativeContract.DVITA.Call(snapshot, "getCandidates");
        ...
        {
            var snapshot = _snapshot.CreateSnapshot();
            var result = NativeContract.DVITA.GetCandidates(snapshot);
            result.Length.Should().Be(0);

            StorageKey key = NativeContract.DVITA.CreateStorageKey(33, ECCurve.Secp256r1.G);
            snapshot.Add(key, new StorageItem(new CandidateState()));
            NativeContract.DVITA.GetCandidates(snapshot).Length.Should().Be(1);
        }
        ...
        {
            var snapshot = _snapshot.CreateSnapshot();
            var committee = NativeContract.DVITA.GetCommittee(snapshot);
            var point = committee[0].EncodePoint(true);

            // Prepare Prefix_VoterRewardPerCommittee
            var storageKey = new KeyBuilder(NativeContract.DVITA.Id, 23).Add(committee[0]).AddBigEndian(20);
            snapshot.Add(storageKey, new StorageItem(new BigInteger(1000)));

            // Prepare Candidate
            storageKey = new KeyBuilder(NativeContract.DVITA.Id, 33).Add(committee[0]);
            snapshot.Add(storageKey, new StorageItem(new CandidateState { Registered = true, Votes = BigInteger.One }));

            storageKey = new KeyBuilder(NativeContract.DVITA.Id, 23).Add(committee[0]);
        ...
        {
            var snapshot = TestBlockchain.GetTestSnapshot();
            var result = (VM.Types.Array)NativeContract.DVITA.Call(snapshot, "getCommittee");
        ...
        {
            var snapshot = _snapshot.CreateSnapshot();
            var result = NativeContract.DVITA.ComputeNextBlockValidators(snapshot, ProtocolSettings.Default);
            result[0].ToArray().ToHexString().Should().Be("02486fd15702c4490a26703112a5cc1d0923fd697a33406bd5a1c00e0013b09a70");
            result[1].ToArray().ToHexString().Should().Be("024c7b7fb6c310fccf1ba33b082519d82964ea93868d676662d4a59ad548df0e7d");
            result[2].ToArray().ToHexString().Should().Be("02aaec38470f6aad0042c6e877cfd8087d2676b0f516fddd362801b9bd3936399e");
            result[3].ToArray().ToHexString().Should().Be("03b209fd4f53a7170ea4444e0cb0a6bb6a53c2bd016926989cf85f9b0fba17a70c");
        ...
        {
            var snapshot = _snapshot.CreateSnapshot();
            NativeContract.DVITA.TotalSupply(snapshot).Should().Be(new BigInteger(1000000000));
        }
        ...
        public void TestEconomicParameter()
        {
            ...
            NativeContract.DVITA.UnclaimedGas(snapshot, UInt160.Zero, persistingBlock.Index + 2).Should().Be(650);
        }
        ...
        public void TestClaimGas()
        {
            ...
            for (var i = 0; i < ProtocolSettings.Default.CommitteeMembersCount; i++)
            {
                ...
                snapshot.Add(new KeyBuilder(NativeContract.DVITA.Id, 33).Add(member), new StorageItem(new CandidateState()
                {
                    Registered = true,
                    Votes = 200 * 10000
                }));
            ...
            snapshot.GetOrAdd(new KeyBuilder(NativeContract.DVITA.Id, 14), () => new StorageItem()).Value = BinarySerializer.Serialize(cachedCommittee.ToStackItem(null), 4096);

            var item = snapshot.GetAndChange(new KeyBuilder(NativeContract.DVITA.Id, 1), () => new StorageItem());
            ...
            NativeContract.DVITA.BalanceOf(snapshot, Contract.CreateSignatureContract(accountA).ScriptHash).Should().Be(0);

            StorageItem storageItem = snapshot.TryGet(new KeyBuilder(NativeContract.DVITA.Id, 23).Add(accountA).AddBigEndian(1));
            new BigInteger(storageItem.Value).Should().Be(300000000000);

            snapshot.TryGet(new KeyBuilder(NativeContract.DVITA.Id, 23).Add(accountB).AddBigEndian(uint.MaxValue - 1)).Should().BeNull();
            ...
            NativeContract.DVITA.BalanceOf(snapshot, Contract.CreateSignatureContract(committee[1]).ScriptHash).Should().Be(0);

            storageItem = snapshot.TryGet(new KeyBuilder(NativeContract.DVITA.Id, 23).Add(committee[1]).AddBigEndian(1));
            new BigInteger(storageItem.Value).Should().Be(300000000000);
            ...
            NativeContract.DVITA.BalanceOf(snapshot, Contract.CreateSignatureContract(committee[2]).ScriptHash).Should().Be(0);

            storageItem = snapshot.TryGet(new KeyBuilder(NativeContract.DVITA.Id, 23).Add(committee[2]).AddBigEndian(22));
            new BigInteger(storageItem.Value).Should().Be(300000000000 * 2);

            // Claim GAS

            var account = Contract.CreateSignatureContract(committee[2]).ScriptHash;
            snapshot.Add(new KeyBuilder(NativeContract.DVITA.Id, 20).Add(account), new StorageItem(new NeoAccountState
            {
                BalanceHeight = 3,
                Balance = 200 * 10000 - 2 * 100,
                VoteTo = committee[2]
            }));
            NativeContract.DVITA.BalanceOf(snapshot, account).Should().Be(1999800);
            BigInteger value = NativeContract.DVITA.UnclaimedGas(snapshot, account, 29 + 3);
            value.Should().Be(1999800 * 30000000000 / 100000000L + (1999800L * 10 * 5 * 29 / 1000));
        }
        ...
        {
            var snapshot = _snapshot.CreateSnapshot();
            NativeContract.DVITA.UnclaimedGas(snapshot, UInt160.Zero, 10).Should().Be(new BigInteger(0));
        ...
        internal (bool State, bool Result) Transfer4TesingOnBalanceChanging(BigInteger amount, bool addVotes)
        {
            ...
            if (addVotes)
            {
                ...
                snapshot.Add(NativeContract.DVITA.CreateStorageKey(33, ECCurve.Secp256r1.G), new StorageItem(new CandidateState()));
            }
            else
            {
                snapshot.Add(CreateStorageKey(20, from.ToArray()), new StorageItem(new NeoAccountState
            ...
            sb.EmitDynamicCall(NativeContract.DVITA.Hash, "transfer", from, UInt160.Zero, amount, null);
        ...
        internal static (BigInteger Value, bool State) Check_GetGasPerBlock(DataCache snapshot, Block persistingBlock)
        {
            ...
            using var script = new ScriptBuilder();
            script.EmitDynamicCall(NativeContract.DVITA.Hash, "getGasPerBlock");
            engine.LoadScript(script.ToArray());
        ...
        {
            UInt160 committeeMultiSigAddr = NativeContract.DVITA.GetCommitteeAddress(snapshot);
            ...
            var script = new ScriptBuilder();
            script.EmitDynamicCall(NativeContract.DVITA.Hash, "setGasPerBlock", gasPerBlock);
            engine.LoadScript(script.ToArray());
        ...
        internal static (bool State, bool Result) Check_Vote(DataCache snapshot, byte[] account, byte[] pubkey, bool signAccount, Block persistingBlock)
        {
            ...
            using var script = new ScriptBuilder();
            script.EmitDynamicCall(NativeContract.DVITA.Hash, "vote", account, pubkey);
            engine.LoadScript(script.ToArray());
        ...
        internal static (bool State, bool Result) Check_RegisterValidator(DataCache snapshot, byte[] pubkey, Block persistingBlock)
        {
            ...
            using var script = new ScriptBuilder();
            script.EmitDynamicCall(NativeContract.DVITA.Hash, "registerCandidate", pubkey);
            engine.LoadScript(script.ToArray());
        ...
        internal static ECPoint[] Check_GetCommittee(DataCache snapshot, Block persistingBlock)
        {
            ...
            using var script = new ScriptBuilder();
            script.EmitDynamicCall(NativeContract.DVITA.Hash, "getCommittee");
            engine.LoadScript(script.ToArray());
        ...
        internal static (BigInteger Value, bool State) Check_UnclaimedGas(DataCache snapshot, byte[] address, Block persistingBlock)
        {
            ...
            using var script = new ScriptBuilder();
            script.EmitDynamicCall(NativeContract.DVITA.Hash, "unclaimedGas", address, persistingBlock.Index);
            engine.LoadScript(script.ToArray());
        ...
        internal static (bool State, bool Result) Check_UnregisterCandidate(DataCache snapshot, byte[] pubkey, Block persistingBlock)
        {
            ...
            using var script = new ScriptBuilder();
            script.EmitDynamicCall(NativeContract.DVITA.Hash, "unregisterCandidate", pubkey);
            engine.LoadScript(script.ToArray());
        ...
        internal static (BigInteger balance, BigInteger height, byte[] voteto) GetAccountState(DataCache snapshot, UInt160 account)
        {
            ...
            using var script = new ScriptBuilder();
            script.EmitDynamicCall(NativeContract.DVITA.Hash, "getAccountState", account);
            engine.LoadScript(script.ToArray());
