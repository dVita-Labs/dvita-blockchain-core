...
namespace Neo.UnitTests.Wallets
{
    ...
    public class UT_Wallet
    {
        ...
        public void TestMakeTransaction1()
        {
            ...
            key = NativeContract.DVITA.CreateStorageKey(20, account.ScriptHash);
            var entry2 = snapshot.GetAndChange(key, () => new StorageItem(new DvitaToken.NeoAccountState()));
            entry2.GetInteroperable<DvitaToken.NeoAccountState>().Balance = 10000 * NativeContract.DVITA.Factor;
            ...
            tx = wallet.MakeTransaction(snapshot, new TransferOutput[]
            {
                new TransferOutput()
                {
                     AssetId = NativeContract.DVITA.Hash,
                     ScriptHash = account.ScriptHash,
                     Value = new BigDecimal(BigInteger.One,8),
                     Data = "Dec 12th"
                }
            ...
            entry2.GetInteroperable<DvitaToken.NeoAccountState>().Balance = 0;
            snapshot.Commit();
        }
