...
{
    /// <summary>
    /// Represents the DVITA token in the NXA/DVITA system.
    /// </summary>
    public sealed class DvitaToken : FungibleToken<DvitaToken.NeoAccountState>
    {
        public override string Symbol => "DVITA";
        public override byte Decimals => 0;

        /// <summary>
        /// Indicates the total amount of DVITA.
        /// </summary>
        public BigInteger TotalAmount { get; }

        /// <summary>
        /// Indicates the effective voting turnout in DVITA. The voted candidates will only be effective when the voting turnout exceeds this value.
        /// </summary>
        ...
        private const byte DvitaHolderRewardRatio = 0;
        private const byte UnclaimedGasRatio = 10;
        private const byte CommitteeRewardRatio = 10;
        private const byte VoterRewardRatio = 80;

        internal DvitaToken()
        {
            this.TotalAmount = 1_000_000_000 * Factor;
        }
        ...
        private BigInteger CalculateBonus(DataCache snapshot, ECPoint vote, BigInteger value, uint start, uint end)
        {
            ...
            BigInteger unclaimedReward = CalculateUnclaimedGas(snapshot, value, start, end);
            return unclaimedReward + value * (endRewardPerNeo - startRewardPerNeo) / 1000000000L;
        }

        private BigInteger CalculateUnclaimedGas(DataCache snapshot, BigInteger value, uint start, uint end)
        {
            BigInteger sum = 0;
            ...
            return value * sum * UnclaimedGasRatio / 100 / TotalAmount;
        }
        ...
        private BigInteger CalculateNeoHolderReward(DataCache snapshot, BigInteger value, uint start, uint end)
        {
            ...
            return value * sum * DvitaHolderRewardRatio / 100 / TotalAmount;
        }
        ...
        internal override async ContractTask PostPersist(ApplicationEngine engine)
        {
            ...
            {
                BigInteger voterRewardOfEachCommittee = gasPerBlock * VoterRewardRatio * 1000000000L * m / (m + n) / 100; // Zoom in 1000000000 times, and the final calculation should be divided 1000000000L
        ...
        /// <summary>
        /// Represents the account state of <see cref="DvitaToken"/>.
        /// </summary>
